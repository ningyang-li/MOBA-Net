...
def dice_recall_per_class(gt_mask, pred_mask, num_classes):
    ...
    for cls in range(num_classes):
        ...
        inter = (gt_cls & pred_cls).sum()
        union = (gt_cls | pred_cls).sum()
        gt_sum = gt_cls.sum()
        # Dice = 2*TP / (TP+FP+TP+FN)
        dice.append((2. * inter + 1e-7) / (union + inter + 1e-7))
        # Recall = TP / (TP+FN)
        recall.append((inter + 1e-7) / (gt_sum + 1e-7))
    return np.array(dice), np.array(recall)
...
class SemSegEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            # single class
            output = (output["sem_seg"] >= self.sem_threshold).squeeze().to(self._cpu_device)
    ...
    def evaluate(self):
        ...
        * Pixel Accuracy (pACC)
        ...
        recall = np.full(self._num_classes, np.nan, dtype=float)
        for i in range(self._num_classes):
            if pos_gt[i] > 0:
                # Dice = 2*TP / (2*TP + FP + FN) = 2*TP / (pos_gt + pos_pred)
                dice[i] = (2. * tp[i]) / (pos_gt[i] + pos_pred[i] + 1e-7)
                # Recall = TP / (TP + FN) = TP / pos_gt
                recall[i] = tp[i] / (pos_gt[i] + 1e-7)
