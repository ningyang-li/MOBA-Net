...
from moba_net.utils import box_ops
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
                 num_points, oversample_ratio, importance_sample_ratio,dn="no",dn_losses=[], 
                 panoptic_on=False, semantic_ce_loss=False, use_moe_losses=True):
    ...
    def get_loss_moe(self, outputs):
        '''
        return losses of all mos and moe layers
        '''
        losses = {}
        losses['loss_topk'] = outputs['topk_loss']
        losses['loss_switch'] = outputs['switch_loss']
        losses['loss_z'] = outputs['z_loss']
    ...
    def forward(self, outputs, targets, mask_dict=None):
        ...
        if self.use_moe_losses:
            losses.update(self.get_loss_moe(outputs))
