...
from ..moe import DST, MoFFN


def build_dst_encoder(cfg, input_shape):
    """
...
class MSDeformAttnTransformerEncoderOnly(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 n_experts=4, k=1, n_points_dst=[1, 2, 3, 4, 5, 6, 7, 8], n_fixed_points=4,
    ...
    def forward(self, srcs, masks, pos_embeds):
        ...
        # encoder
        memory, losses_moe = self.encoder(src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten, mask_flatten)
...
class MSDeformAttnTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        if attn_type == "dst":
            # dst
            self.self_attn = DST(n_experts=n_experts, k=k, d_model=d_model, n_levels=n_levels, n_heads=n_heads, n_points=n_points_dst, 
                                 n_fixed_points=n_fixed_points, noisy_gating=noisy_gating, acc_aux_loss=acc_aux_loss)
        else:
            # self attention
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        loss_dst = {'topk_loss': torch.zeros((1,), device=src.device), 'switch_loss': torch.zeros((1,), device=src.device), 'z_loss': torch.zeros((1,), device=src.device)}
        ...
        if self.attn_type == "dst":
            src2, loss_dst = self.self_attn(self.with_pos_embed(src, pos), reference_points, src, spatial_shapes, level_start_index, padding_mask)
        else:
            # msda
        ...
        if self.ffn_type == "moffn":
            src3, loss_moe = self.ffn(src)
        else:
            src3 = self.ffn(src)
        src = self.norm2(src + src3)

        return src, loss_dst, loss_moe
...
class MSDeformAttnTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        ...
        for _, layer in enumerate(self.layers):
            output, loss_dst, loss_moe = layer(output, pos, reference_points, spatial_shapes, level_start_index, padding_mask)
            # aggregate losses of all dst and moe layers
            for k in losses_moe.keys():
                losses_moe[k] += loss_dst[k]
...
class DST_Encoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            k            number of selected expert, k must be 1 to avoid to modify the cuda implemtntation of MSDA
            n_points_dst number of sampling points per attention head per feature level in an expert
            n_fixed_points  number of sampling points using fixed pattern
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ret["n_experts"] = cfg.MODEL.MOE.N_EXPERTS_ENC
        ret["k"] = cfg.MODEL.MOE.K_ENC
        ret["n_points_dst"] = cfg.MODEL.MOE.N_POINTS
        ret["n_fixed_points"] = cfg.MODEL.MOE.N_FIXED_POINTS
