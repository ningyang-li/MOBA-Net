...
from ..moe import MOBA, MoFFN, MoS
from .ops.modules import MSDeformAttn


def build_moba_encoder(cfg, input_shape):
    """
...
# Transformer encoder in deformable detr
class TransformerEncoderOnly(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds):
        ...
        # encoder    
        memory, losses_moe = self.encoder(src=src_flatten, mask_features=mask_flatten, pos=lvl_pos_embed_flatten, level_start_index=level_start_index,
                                          spatial_shapes=spatial_shapes, valid_ratios=valid_ratios, padding_mask=mask_flatten)

        return memory, spatial_shapes, level_start_index, losses_moe
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        if attn_type == "moba":
            self.self_attn = MOBA(n_experts=n_experts, k=k, d_model=d_model, n_levels=n_levels, n_heads=n_heads,
    ...
    def forward(self, src, mask_features, pos, attn_mask_target_size, layer_id, reference_points, spatial_shapes=[], level_start_index=[], padding_mask=None):
        loss_mos = {'topk_loss': torch.zeros((1,), device=src.device), 'switch_loss': torch.zeros((1,), device=src.device), 'z_loss': torch.zeros((1,), device=src.device)}
        loss_moe = {'topk_loss': torch.zeros((1,), device=src.device), 'switch_loss': torch.zeros((1,), device=src.device), 'z_loss': torch.zeros((1,), device=src.device)}
        # self attention
        if self.attn_type == "moba":
            src2, loss_mos, gate_metrics, attn_weights = self.self_attn(query=self.with_pos_embed(src, pos),
                                                                        input_flatten=src,
        ...
        if self.ffn_type == "moe":
            src3, loss_moe, _ = self.ffn(src)
        else:
            src3 = self.ffn(src)
        src = self.norm2(src + src3)

        return src, loss_mos, loss_moe
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, mask_features, pos=None, level_start_index=[], spatial_shapes=[], valid_ratios=None, padding_mask=None):
        # inputs are all flattened
        output = src
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        # transfer input from tensor to list
        level_outputs = []
        num_feature_levels = self.layers[0].num_feature_levels
        for i in range(num_feature_levels):
            start = level_start_index[i]
            end = level_start_index[i+1] if i < num_feature_levels - 1 else src.shape[1]
            level_outputs.append(src[:, start:end])  # 每个元素是独立的 tensor
        
        losses_moe = {'topk_loss': torch.zeros((1,), device=src.device), 'switch_loss': torch.zeros((1,), device=src.device), 'z_loss': torch.zeros((1,), device=src.device)}

        for layer_id, layer in enumerate(self.layers):
            # use certain scale of features successively
            level_index = layer_id % num_feature_levels
            attn_mask_target_size = (int(spatial_shapes[(layer_id) % num_feature_levels][0]), int(spatial_shapes[(layer_id) % num_feature_levels][0]))
            query_start_index = level_start_index[level_index]
            query_end_index = level_start_index[level_index+1] if level_index != num_feature_levels-1 else output.shape[1]
            
            # fetch query from flattened inputs
            cur_output = level_outputs[level_index]
            # cur_output = output[:, query_start_index:query_end_index]
            cur_mask_features = output[:, level_start_index[-1]:] # always use the 4x scale 
            cur_pos = pos[:, query_start_index:query_end_index]

            # [N, query, d_model], [N, query], [N, query, d_model], [h, w], int
            # for self-attention "cur_output" will be "cur_mask_features"
            cur_output, loss_mos, loss_moe = layer(src=cur_output, mask_features=cur_mask_features, pos=cur_pos, attn_mask_target_size=attn_mask_target_size,
                                                   layer_id=layer_id, reference_points=reference_points, spatial_shapes=spatial_shapes, 
                                                   level_start_index=level_start_index, padding_mask=padding_mask)
            # output[:, query_start_index:query_end_index] = cur_output
            level_outputs[level_index] = cur_output
        ...
        return output, losses_moe


@SEM_SEG_HEADS_REGISTRY.register()
class MOBA_Encoder(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            attn_type    type of attention in encoder
            ffn_type     type of ffn in encoder
        ...
        self.MOBA_num_feature_levels = num_feature_levels  # always use 3 scales
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ret["boltzmann_sampling"] = {"mask_threshold": cfg.MODEL.DECODER.BOLTZMANN.MASK_THRESHOLD,
                                     ...
                                     "use_sparse": cfg.MODEL.DECODER.BOLTZMANN.USE_SPARSE}
    ...
    def forward_features(self, features, masks):
        ...
        if self.feature_order != 'low2high':
            srcs = srcsl
            pos = posl
        # features; spatial shape of each scale; start index of each scale in patches list
        y, spatial_shapes, level_start_index, losses_moe = self.transformer(srcs, masks, pos)
        bs = y.shape[0]

        # split features of each scale from y
