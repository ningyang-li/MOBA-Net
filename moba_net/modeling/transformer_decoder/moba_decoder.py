...
from .moba_decoder_layer import TransformerDecoder, TransformerDecoderLayer
...
def build_moba_decoder(cfg, in_channels, mask_classification=True):
    """
...
class MOBA_Decoder(nn.Module):
    ...
    def __init__(
            ...
            *,
            ...
            shared_gate: bool,
            n_ranks: list,
            multiple_cls_eval: bool,
            inside_box_head: bool,
    ):
        """
        ...
        Args:
            ...
            attn_type    type of attention in decoder
            ffn_type     type of ffn in decoder
            n_experts    number of experts (for both offset layer and attention layer)
            k            number of selected expert
            boltzmann_sampling  parameters of boltzmann attention module
            noisy_gating whether add noise to clear logits of gate
            acc_aux_loss save the usage of experts
            shared_gate  whether share weights of gate of attn to ffn
            n_ranks      rank of each expert
            multiple_cls_eval    use the fusion of cls outputs of multiple layers
            inside_box_head  use box heads in experts
        """
        super().__init__()
        ...
        decoder_layer = TransformerDecoderLayer(hidden_dim, dim_feedforward,
                                                dropout, activation,
                                                self.num_feature_levels, nhead,
                                                # moe
                                                attn_type=attn_type,
                                                ffn_type=ffn_type,
                                                n_experts=n_experts,
        ...
        # Cross-Decoder Query Fusion layer
        self.layer_fusion_weights = nn.Parameter(torch.ones(self.num_layers,))
        self.multiple_cls_eval = multiple_cls_eval
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret["nheads"] = cfg.MODEL.DECODER.NHEADS
        ret["dim_feedforward"] = cfg.MODEL.DECODER.DIM_FEEDFORWARD
        ret["dec_layers"] = cfg.MODEL.DECODER.DEC_LAYERS
        ret["enforce_input_project"] = cfg.MODEL.DECODER.ENFORCE_INPUT_PROJ
        ret["mask_dim"] = cfg.MODEL.SEM_SEG_HEAD.MASK_DIM
        ret["two_stage"] =cfg.MODEL.DECODER.TWO_STAGE
        ret["initialize_box_type"] = cfg.MODEL.DECODER.INITIALIZE_BOX_TYPE  # ['no', 'bitmask', 'mask2box']
        ret["dn"]=cfg.MODEL.DECODER.DN
        ret["noise_scale"] =cfg.MODEL.DECODER.DN_NOISE_SCALE
        ret["dn_num"] =cfg.MODEL.DECODER.DN_NUM
        ret["initial_pred"] =cfg.MODEL.DECODER.INITIAL_PRED
        ret["learn_tgt"] = cfg.MODEL.DECODER.LEARN_TGT
        ...
        ret["attn_type"] = cfg.MODEL.MOE.ATTN_TYPE_DEC
        ret["ffn_type"] = cfg.MODEL.MOE.FFN_TYPE_DEC
        ret["n_experts"] = cfg.MODEL.MOE.N_EXPERTS_DEC
        ret["k"] = cfg.MODEL.MOE.K_DEC
        ret["boltzmann_sampling"] = {"mask_threshold": cfg.MODEL.DECODER.BOLTZMANN.MASK_THRESHOLD,
                                     "do_boltzmann": cfg.MODEL.DECODER.BOLTZMANN.DO_BOLTZMANN,
                                     "sample_ratio": cfg.MODEL.DECODER.BOLTZMANN.SAMPLE_RATIO,
                                     "base_temp": cfg.MODEL.DECODER.BOLTZMANN.BASE_TEMP,
                                     "gaussian_sampling": cfg.MODEL.DECODER.BOLTZMANN.GAUSSIAN_SAMPLING}
        ret["noisy_gating"] = cfg.MODEL.MOE.NOISY_GATING
        ret["acc_aux_loss"] = cfg.MODEL.MOE.ACC_AUX_LOSS
        ret["shared_gate"] = cfg.MODEL.MOE.SHARED_GATE
        ret["n_ranks"] = cfg.MODEL.MOE.N_RANKS
        ret["multiple_cls_eval"] = cfg.MODEL.DECODER.TEST.MULTIPLE_CLS_EVAL
        ret["inside_box_head"] = cfg.MODEL.MOE.INSIDE_BOX_HEAD
        return ret
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        if self.two_stage:
            ...
            refpoint_embed = refpoint_embed_undetach
        ...
        hs, references, losses_moe, outputs_mask_list, out_boxes = self.decoder(
            ...
            tgt_mask=tgt_mask,
            memory_mask=mask_features,
        )

        if self.inside_box_head == False:
            # from hidden get boxes
            if self.initial_pred:
        ...
        for i, output in enumerate(hs):
            outputs_class, outputs_mask = self.forward_prediction_heads(output.transpose(0, 1), mask_features, pred_mask=False)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask_list[i]) # add mask predicted by moba module
            
        # === Cross-Decoder Inference Fusion ===
        if self.training:
            hs_stacked = torch.stack(hs, dim=0)  # [4, N, Len_q, d_model]
            ref_stacked = torch.stack(references[-len(hs):], dim=0)
            outputs_mask_stacked = torch.stack(outputs_mask_list, dim=0)
            mask_size = outputs_mask_stacked.shape[-1]
            outputs_mask_stacked = outputs_mask_stacked.flatten(-2)
            outputs_box_stacked = torch.stack(out_boxes[-len(hs):], dim=0) # do not fuse initial reference points

            fusion_weights = F.softmax(self.layer_fusion_weights, dim=0).view(-1, 1, 1, 1)
            hs_fused = (hs_stacked * fusion_weights).sum(dim=0)
            ref_fused = (ref_stacked * fusion_weights).sum(dim=0)
            outputs_mask_fused = (outputs_mask_stacked * fusion_weights).sum(dim=0).unflatten(-1, (mask_size, mask_size))
            outputs_box_fused = (outputs_box_stacked * fusion_weights).sum(dim=0)

            # outputs_class_f, outputs_mask_f = self.forward_prediction_heads(hs_fused.transpose(0, 1), mask_features, False)
            predictions_class_stacked = torch.stack(predictions_class[-len(hs):], dim=0) # do not fuse initial reference points
            outputs_class_f = (predictions_class_stacked * fusion_weights).sum(dim=0)

            # replace the final output
            # predictions_class[-1] = outputs_class_f
            # predictions_mask[-1] = outputs_mask_fused
            # out_boxes[-1] = outputs_box_fused
    
            # append the fused results to the end
            predictions_class.append(outputs_class_f)
            predictions_mask.append(outputs_mask_fused)
            out_boxes.append(outputs_box_fused)
        else:
            pass        
        ...
        # # CDIF
        # out = {
        #     'pred_logits': predictions_class[-1] if self.training else torch.cat((interm_outputs['pred_logits'], torch.stack(predictions_class[1:], dim=1).flatten(1, 2)), dim=1),
        #     'pred_masks': predictions_mask[-1] if self.training else torch.cat((interm_outputs['pred_masks'], torch.stack(predictions_mask[1:], dim=1).flatten(1, 2)), dim=1),
        #     'pred_boxes': out_boxes[-1] if self.training else torch.cat((interm_outputs['pred_boxes'], torch.stack(out_boxes[1:], dim=1).flatten(1, 2)), dim=1),
        #     'fused_logits': predictions_class[-1],
        #     'fused_masks': predictions_mask[-1],
        #     'aux_outputs': self._set_aux_loss(
        #         predictions_class[:-1] if self.mask_classification else None, predictions_mask[:-1],out_boxes[:-1]
        #     )
        # }

        # Optimization rely on the fusion layer
        out = {
            'pred_logits': predictions_class[-1] if self.training else interm_outputs['pred_logits'],
            'pred_masks': predictions_mask[-1] if self.training else interm_outputs['pred_masks'],
            'pred_boxes': out_boxes[-1] if self.training else interm_outputs['pred_boxes'],
            'fused_logits': interm_outputs['pred_logits'],
            'fused_masks': interm_outputs['pred_masks'],
            'aux_outputs': self._set_aux_loss(
                predictions_class[:-1] if self.mask_classification else None, predictions_mask[:-1],out_boxes[:-1]
            )
        }
        ...
        return out, mask_dict, losses_moe
    ...
    def boltzmann_forward_prediction_experts(self, output, mask_features, pred_mask=True):
        pass
